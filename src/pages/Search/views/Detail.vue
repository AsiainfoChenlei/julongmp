<template lang="html">
  <div class="detail">
    ...
    <!--<v-content></v-content>-->
    <v-assess v-if="goods_id" :goodsid="goods_id"></v-assess>
...
export default {
    ...
    setActive () {
      ...
      self.$store.dispatch('getGoodsInfo', {
        ...
        if (result.code) {
          ...
          self.info.intro = result.msg.name
          self.goodsid = result.msg.id
          self.property = result.msg.stock
          self.goods_id = self.query.shop_id+'_'+self.query.goods_id
          // 限制最大长度
          ...
          if (self.property.length > 0) {
            ...
            self.$store.dispatch('getSignPackage', {'url': window.location.href}).then(function (res) {
    ...
    param () {
      ...
      if (self.$route.query.serial) {
        let query = {
          ...
          codeid: self.$route.query.codeid,
          is_sale: self.$route.query.is_sale
        }
        self.$set(self, 'query', query)
        if (self.$route.query.codeid === 'null' || self.$route.query.codeid === null || typeof (self.$route.query.codeid) === 'undefined') {
          console.log('null1')
        } else {
          Cookies.set('DETAILCODEID', self.$route.query.codeid)
        }
      }
    }
  },
  ...
  //      url:'/shopdetail'
  //    }).then((response) => {
  //      this.dataswip = response.data.swiper;
