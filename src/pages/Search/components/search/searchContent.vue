<template>
  <div v-if="identifier==='goods'&&identifier===currrent"
       class="content"
       v-infinite-scroll="loadMore"
       infinite-scroll-disabled="allLoad"
       infinite-scroll-distance="10">
    <div style="display:none"
         v-for="(item,index) in list"
         :key="item.id">
      <img :id="index"
           :src="$helper.getRealPicURL(item.cover,item.company_id,item.shop_id) | preloadImage(item,that)" />
    </div>
    <div class="leftList">
      <div v-if="leftList.length > 0"
        ...
        <a @click="godetail(item.goods_id,item.company_id,item.serial,item.shop_id)">
          <img class="itemContainer1 img"
               :src="$helper.getRealPicURL(item.cover,item.company_id,item.shop_id,'wcut_h400')">
          ...
          <div class="itemContainer1 price">
            ...
            <span v-if="parseInt(item.max_price)> parseInt(item.min_price)"
                  class="price">~{{item.max_price}}</span>
          </div>
  ...
  <div v-else-if="identifier==='shop'&&identifier===currrent"
    ...
    <div v-for="(item ,index) in list"
      ...
      <a @click="goshophome (item.id, item.company_id)">
        <img class="cover"
             :src="$helper.getRealPicURL(item.face,1,1,'w100_h100')">
        <div class="info">
