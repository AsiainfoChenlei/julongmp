...
const actions = {
  ...
  createOrder_common ({commit}, param) {
    return vm.$api({
      method: 'post',
  ...
  getUpToken ({commit}, param) {
    return vm.$api({
      method: 'get',
      url: Util.ajaxUrl + ':8003/admin/Api/UpToken?token=' + vm.$localStorage.get('TOKEN')
    })// 直接返回对象给页面调用
  },
  uploadImageTo7N ({commit}, parm) {
    return vm.$api({
      url: 'http://up.qiniu.com', // 被上传的目标地址,
      method: 'post',
