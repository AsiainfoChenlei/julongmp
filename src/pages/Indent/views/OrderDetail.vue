<template lang="html">
  ...
  <div class="orderdetial" >
    ...
    <span  v-else  class="icon-go"  @click="$router.go(-1)"></span>
     <div slot="title" class="or-header">订单详情</div>
  </header>

    <div class="order1">
      <span class="span-1">订单状态</span>
      <span class="span-2-1"  v-if="view && view.status_list && Array.isArray(view.status_list) && view.status_list.length > 0">{{view.status_list[0].status}}</span>
    </div>
    ...
    <div class="order-huabian"></div>
    <div class="order3">
      <div class="order3-1" @click="shop"><span class="order3-span"  v-if="view && view.status_list && Array.isArray(view.status_list) && view.status_list.length > 0">{{view.shop_info.shop_name}}</span><i class="icon-go"></i></div>
      <div class="order3-2">

        <div class="order3-2-1" v-if="goodsrows && Array.isArray(goodsrows) && goodsrows.length > 0"  v-for="(goods,index) in goodsrows" :key="index" @click="detail(goods.goods_id,goodsrows[0].serial)">
          <div class="order-1-img">
            <img class="order-img" v-if="view && view.status_list && Array.isArray(view.status_list) && view.status_list.length > 0" :src="$helper.getRealPicURL(goods.stock.images,view.shop_info.company_id,view.shop_id)" />
          </div>
      ...
      <div class="order3-4">
        <div class="order3-div order3-div-1"><span class="div-1">总计</span><span class="order3-span-1"   v-if="goodsrows && Array.isArray(goodsrows) && goodsrows.length > 0">￥{{goodsrows[0].stock.price}}</span></div>
    ...
    <div class="order4">
      ...
      <div class="order4-1">
        ...
        <span v-if="view.send_serial!=''" class="order4-1-span" >快递图片：
            <img style="width:80%" v-if="view && view.status_list && Array.isArray(view.status_list) && view.status_list.length > 0"  v-bind:src="$helper.getRealPicURL(view.send_serial,+view.shop_info.company_id,+view.shop_info.id)" alt="">
        </span>
      </div>
    </div>
    ...
    <div class="bottoms">
      ...
      <v-order v-if="view && view.status_list && Array.isArray(view.status_list) && view.status_list.length > 0"  :type="+view.status_list[0].status_code" :serial="view.system_serial" v-on:doActive="activeSuccess"></v-order>
      <mt-popup
...
import {Spinner, Popup, MessageBox, Picker} from 'mint-ui'
...
export default {
  ...
  methods: {
    ...
    detail (v, serial) {
      ...
      location.assign('shop.html#/shopdetail?shop_id=' + self.view.shop_id + '&company_id=' + self.view.shop_info.company_id + '&goods_id=' + v + '&serial=' + serial)
    },
    activeSuccess (msg) {
      const self = this
    ...
    QQurl1(){
      MessageBox({
        title: '联系卖家',
        message: this.view.shop_info.contact,
        showCancelButton: true,
        confirmButtonText: '拨号',
        cancelButtonText: '取消'
      }).then(action=> {
        if(action== 'confirm'){
          window.location.href = 'tel://'+this.view.shop_info.contact;
        }else {
