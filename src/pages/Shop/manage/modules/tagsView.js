const tagsView = {
  ...
  mutations: {
    ...
    ADD_VISITED_VIEWS6: (state, view) => {
      ...
      state.visitedViews.forEach(v => {
        console.log('v', v)
        if (v.path === view.path && +v.query.company_id === +view.query.company_id && v.query.shop_id === view.query.shop_id && v.query.select === view.query.select) {
          v.meta.position = view.meta.position
          console.log('v.meta.position', v.meta.position)
        }
      })
      console.log('agoview', view)
      if (state.visitedViews.some(v => v.path === view.path && +v.query.company_id === +view.query.company_id && v.query.shop_id === view.query.shop_id && v.query.select === view.query.select)) {
