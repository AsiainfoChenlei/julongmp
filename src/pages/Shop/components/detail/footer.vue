<template lang="html">
  <footer class="footer">
    ...
    <a  @click="QQurl1" class="kefu">联系商家</a>
      ...
      <div class="pop-2">
        ...
        <div class="pop2">
         <!-- <span class="pop2-color" v-for="(k,i) in view.stock" :class="{'color-active':i==firstselect}" @click="colChose(i)">{{k.standard}}</span>-->
              <span class="pop2-color" v-for="(k,i) in view.stock" :key="k.standard" :class="{'color-active':i==colSelected}" @click="colChose(i)">{{k.standard}}</span>
        </div>
      </div>
      <div class="paycar" @click="addOrder">立即结算</div>
    </mt-popup>
    <div class="toast-div" v-show="toas"><div class="toast-img" @click="tauch"></div></div>
  </footer>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      popvisible: false,
      // firstselect: 0,
      //addTjImage: {background: 'url(' + require('static/images/ts.png') + ')', 'background-size': '100% 100%'},
      property: [],
      colSelected: 0,
      toas: false,
      chref: 'mqqwpa://im/chat?chat_type=wpa&uin= ' + this.view.shop_info.qq + '&version=1&src_type=web&web_src=oicqzone.com'
    }
  },
  computed: {
    //      QQurl() {
    ...
    //          Toast("抱歉,当前游览器不支持QQ跳转，请转到普通游览器或下载巨龙app");
    //          return ;
    //        }
  ...
  methods: {
    ...
    shop () {
      ...
      // return;
      /*   this.$router.push({
          path: 'shop?shop_id=' + self.query.shop_id + '&company_id=' + self.query.company_id
        }) */
    ...
    setRCT () {
      let inFifteenMinutes = new Date(new Date().getTime() + 5 * 60 * 1000) // 5分钟
      console.log('inFifteenMinutes', inFifteenMinutes)
      Cookies.set('DETAILURL', window.location.href, {
...
<style lang="less" scoped>
  ...
  .footer {
    ...
    .kefu {
      ...
      height: 9vw;
      text-align: center;
      background: url("/static/images/kefu1.png") no-repeat;
      margin: 0.8% 2% auto 2%;
    ...
    .toast-div{
      ...
      .toast-img{
        height: 100%;
        width: 100%;
        background: url("~static/images/ts.png");
