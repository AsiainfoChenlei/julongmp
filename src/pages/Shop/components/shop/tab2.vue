<template>
  <div :id="inputId" class="tab">
    <div style="display:none" v-for="(item,index) in list" :key="item.id">
      <img class="tab2-li-img" :id="index" :src="$helper.getRealPicURL(item.cover,query.company_id,query.shop_id) | preloadImage(item,that)" />
    </div>
    <ul class="tab2-ul" v-infinite-scroll="loadMore" infinite-scroll-disabled="isall" infinite-scroll-distance="10">
      <div id="leftColumn" class="left">
        <li v-if="list.length > 0" v-for="(item,index) in leftList" :key="item.id" :flag="item.is_sale" :class="'tab2-li tab2-li'+item.is_sale">
          <router-link :to="{name:'详情页',query:{'shop_name':shop_name,goods_id:item.id,serial:encodeURIComponent(item.serial),company_id:query.company_id,shop_id:query.shop_id,codeid:$localStorage.get('CODEID'),is_sale:item.is_sale}}">
            <img class="tab2-li-img" :id="index" :src="$helper.getRealPicURL(item.cover,query.company_id,query.shop_id)" />
      ...
      <div id="rightColumn" class="right">
        <li v-if="list.length > 0" v-for="(item,index) in rightList" :key="item.id" :class="'tab2-li tab2-li'+item.is_sale">
          <router-link :to="{name:'详情页',query:{'shop_name':shop_name,goods_id:item.id,serial:encodeURIComponent(item.serial),company_id:query.company_id,shop_id:query.shop_id,codeid:$localStorage.get('CODEID'),is_sale:item.is_sale}}">
            <img class="tab2-li-img" :id="index" :src="$helper.getRealPicURL(item.cover,query.company_id,query.shop_id)" />
            <div class="tab2-li-span">
              <span class="li-span-name">{{item.name}}</span>
    ...
    <div class="spinner-center" v-show="!spinnerloading">
      <mt-spinner type="fading-circle"></mt-spinner>
    </div>
    <div v-show="isall" class="tab2-ul-footer">已经是全部啦~~~</div>
  </div>
</template>
...
export default {
  ...
  props: ['shopid', 'id', 'select', 'query', 'shop_name','listid'],
  data () {
    return {
      isall: false,
